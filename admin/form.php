...
<?php 
    ...
    if(isset($_POST['lolos'])){
        $id = $_POST['id'];
        $konfirmasiaja = mysqli_query($conn,"update userdata set status = 'Lolos' WHERE userid=$id");
      ...
      if(isset($_POST['tidaklolos'])){
        $id = $_POST['id'];
        $konfirmasiaja = mysqli_query($conn,"update userdata set status = 'Tidak Lolos' WHERE userid=$id");
...
<head>
    ...
    <title>Formulir Siswa - SMAN 3 Tambun Selatan</title>
...
<body>
                                ...
                                <div class="d-sm-flex justify-content-between align-items-center">
									<h2>Kelola Formulir</h2>
										 ...
										 <table id="dataTable3" class="table table-hover" style="width:100%"><thead class="thead-dark">
											...
											<?php 
											$form=mysqli_query($conn,"SELECT * FROM userdata where status='Proses' OR status='Lolos' OR status='Tidak Lolos'  ORDER BY userdataid DESC");
											$no=1;
											while($b=mysqli_fetch_array($form)){
												...
												<tr>
                                                    ...
                                                    <a class="btn btn-success btn-sm modalConfirmasi" href="" data-toggle="modal" data-id="<?=$b['userid']?>" data-target="#modalkonfirmasi">Konfirmasi</a>
                                                    </td>
                                   ...
                                   <!-- The Modal -->
                                    <div class="modal fade" id="modalkonfirmasi">
                                        ...
                                        <div class="modal-header">
                                                <h5 class="modal-title" id="exampleModalLabel">Konfirmasi Status Siswa</h5>
                                            ...
                                            <!-- Modal footer -->
                                            <div class="modal-footer">
                                            <input type="submit" class="btn btn-success" name="lolos" value="LOLOS">
                                            <input type="submit" class="btn btn-danger" name="tidaklolos" value="TIDAK LOLOS">
                                            </div>
                                            </form>
                                    </div>
	...
	<script>
		$(document).ready(function() {
            $('.modalConfirmasi').on('click',function() {
